...
namespace HttpServerLib
{
    ...
    public class HttpServer : IServer
    {
        ...
        /// 服务器协议
        /// </summary>
        private Protocols Protocol { get; set; }
        ...
        public void Start()
        {
            ...
            this.Log(string.Format("RootPath {0}", ServerRoot));
            try
            {
                while (IsRunning)
                {
        ...
        {            
            ...
            //构造HTTP响应
            HttpResponse response = new HttpResponse(clientStream, Logger);

            //构造HTTP请求
            HttpRequest request = new HttpRequest(Logger);
            if (!request.Parse(clientStream))
            {
                response.SetError(500, "Bad Request");
                return;
            }
            try
            {
